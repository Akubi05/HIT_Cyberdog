...
```
luke@akubi:~$ sudo docker load -i cyberdog_race2026.tar --quiet=false
Loaded image: cyberdog_sim:v2026
```

- 运行 Docker 镜像来创建一个容器
  `sudo docker run -it --privileged=true -e DISPLAY=\$DISPLAY -v/tmp/.X11-unix:/tmp/.X11-unix cyberdog_sim:v2026`
