...
class VisionNode(Node):
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if current_state == 0:
                ...
                self._detect_1(cv_image)   
            elif current_state == 2 or current_state == 7 or current_state == 11 or current_state == 17 or current_state == 2 or current_state == 31 or current_state == 44 or current_state == 48 or current_state == 53: 
                self._show(cv_image)
    ...
    def _detect_1(self, cv_image, ball_color="blue"):
        """
        检测前方一定范围内是否有指定颜色的球
        ...
        with self._lock:
            current_time = time.time()
            self._detected_flags['1'] = detected
            self._last_detection_time = current_time
    ...
    def _detect_4(self, cv_image):
        ...
        with self._lock:
            current_time = time.time()
            self._detected_flags['4'] = detected
            self._last_detection_time = current_time
    ...
    def _detect_5(self, cv_image):
        ...
        # =========================
        with self._lock:
            self._detected_flags['5'] = detected
        ...
        if self.debug_mode:
            ...
            text = f"blue cubes detected: {self._detected_flags['5']}"
    ...
    def _detect_6(self, cv_image):
        ...
        # =========================
        with self._lock:
            # self._detected_flags['6'] = detected
    ...
    @property
    def _1_detected(self):
        with self._lock:
            return self._detected_flags['1']
        ...
        while rclpy.ok():
            ...
            if state_id == 0:
                if current_time - last_cmd_time > cmd_interval:
                    ...
                    msg.vel_des = [0.3, 0, 0.0]  # 前进速度0.3m/s
                    # msg.rpy_des = [-2.5, 0 ,0]
