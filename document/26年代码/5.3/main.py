    ...
    def _detect_lane_center(self, cv_image):
        ...
        def detect_side_line(x_min, x_max, side_name):
            ...
            for line in lines:
                ...
                # 越靠近中线越好：距离中线越小，bias 越大
                center_bias = -abs(ref_x - img_center_x)
                score = length + 0.3 * center_bias
...
class IMUNode(Node):
    ...
    def imu_callback(self, msg):
        """
        从 IMU 四元数中计算 yaw，并写入 app_state.yaw。
        yaw 单位：弧度，范围大约是 [-pi, pi]
        """
...
class RobotApp:
    ...
    def start(self):
        ...
        self.imu_node = IMUNode("imu_node", self.app_state)
    ...
    def _turn_by_yaw(
        ...
        作用：
            让机器狗按照 IMU / app_state.yaw 的反馈转过指定角度。

        返回：
            last_cmd_time, finished

        参数说明：
            current_time:
                当前时间，一般由主循环传入 time.time()
            ...
            turn_mode / turn_gait_id:
                转弯时发送的 mode 和 gait_id

            stop_mode / stop_gait_id:
                停止时发送的 mode 和 gait_id
        """

        # 根据 turn_key 生成“起始 yaw”的属性名
        # 例如 turn_key="state1_turn_180"
        # 那么 start_yaw_attr 就是 "_state1_turn_180_start_yaw"
        start_yaw_attr = f"_{turn_key}_start_yaw"

        # 根据 turn_key 生成“开始时间”的属性名
        # 例如 "_state1_turn_180_start_time"
        start_time_attr = f"_{turn_key}_start_time"

        # 判断这是不是第一次进入这个转弯函数
        # 如果对象 self 上还没有 start_yaw_attr 这个属性
        # 说明当前转弯任务刚开始
        if not hasattr(self, start_yaw_attr):

            # 记录当前 yaw 作为起始角度
            # 后面用 current_yaw - start_yaw 来计算已经转了多少
            setattr(self, start_yaw_attr, self.app_state.yaw)

            # 记录当前时间作为开始时间
            # 后面用于超时保护 max_turn_time
            setattr(self, start_time_attr, current_time)

            # 打印调试信息
            print(
                f"{turn_key}：开始转弯，"
                f"target={math.degrees(target_angle):.1f}度，"
                f"forward_speed={forward_speed:.2f}, "
                f"start_yaw={self.app_state.yaw:.3f}"
            )
        # 取出本次转弯开始时记录的 yaw
        start_yaw = getattr(self, start_yaw_attr)

        # 取出本次转弯开始的时间
        start_time = getattr(self, start_time_attr)

        # 读取当前 yaw
        current_yaw = self.app_state.yaw

        # 计算当前 yaw 相对起始 yaw 的变化量
        # 注意：这里必须做角度归一化
        # 否则 yaw 从 pi 跳到 -pi 时会算错
        turned_angle = self._normalize_angle(current_yaw - start_yaw)

        # 把转过的角度投影到指定方向上
        # 如果 turn_direction = 1.0，正方向转动是有效转角
        # 如果 turn_direction = -1.0，反方向转动是有效转角
        turned_in_direction = turn_direction * turned_angle

        # 如果刚开始 IMU 抖动，导致有效转角变成负数
        # 这里直接压成 0，避免 remaining_angle 变得更大
        if turned_in_direction < 0:
            turned_in_direction = 0.0

        # 计算剩余角度
        # 目标角度 - 已经转过的角度
        remaining_angle = target_angle - turned_in_direction
        ...
        # 判断是否因为角度达到目标而完成
        # 剩余角度小于容忍误差，就认为完成
        finished_by_angle = remaining_angle <= angle_tolerance

        # 判断是否因为超时而强制完成
        # 这是保护机制，防止机器人一直转
        finished_by_timeout = elapsed >= max_turn_time

        # 如果角度达标，或者超时，就停止转弯
        if finished_by_angle or finished_by_timeout:

            # 控制命令不要每次循环都发
            # 只有距离上次发送超过 cmd_interval，才发送停止命令
            if self._tick_ready(current_time, last_cmd_time, cmd_interval):

                # 发送停止命令
                # vel=[0.0, 0.0, 0.0] 表示前进、横移、转向都停止
                self._send_cmd(
                    mode=stop_mode,
                    gait_id=stop_gait_id,
                    vel=[0.0, 0.0, 0.0]
                )
            ...
            print(
                f"{turn_key}：转弯完成，"
                f"turned={math.degrees(turned_in_direction):.1f}度，"
                f"elapsed={elapsed:.2f}s"
            )

            # 删除起始 yaw 临时变量
            # 这样下次再调用同一个 turn_key 时，会重新记录起始 yaw
            if hasattr(self, start_yaw_attr):
                delattr(self, start_yaw_attr)
            ...
            # 返回：
            # last_cmd_time：更新后的命令时间
            # True：表示本次转弯已经完成
            return last_cmd_time, True

        # 如果还没转完，就判断是否需要减速
        # remaining_angle < slow_down_angle 表示快到目标了
        if remaining_angle < slow_down_angle:

            # 接近目标时用较小 yaw 速度，防止冲过头
            yaw_speed = slow_yaw_speed

        else:

            # 离目标还远时用较大 yaw 速度，提高效率
            yaw_speed = fast_yaw_speed

        # 根据方向决定 yaw_speed 的正负
        # turn_direction = 1.0，则 yaw_speed 为正
        # turn_direction = -1.0，则 yaw_speed 为负
        yaw_speed *= turn_direction
        ...
        if self._tick_ready(current_time, last_cmd_time, cmd_interval):
            ...
            self._send_cmd(
                mode=turn_mode,
                gait_id=turn_gait_id,
                vel=[forward_speed, lateral_speed, yaw_speed]
            )
            ...
            print(
                f"{turn_key}：转弯中，"
                f"turned={math.degrees(turned_in_direction):.1f}度，"
                f"remain={math.degrees(remaining_angle):.1f}度，"
                f"vel=[{forward_speed:.2f}, {lateral_speed:.2f}, {yaw_speed:.2f}]"
            )

        # 返回：
        # last_cmd_time：可能被更新，也可能没变
        # False：表示还没有转完，下一轮主循环继续调用
        return last_cmd_time, False
    ...
    def _handle_state_0(self, current_time, last_cmd_time, cmd_interval):
        if self._tick_ready(current_time, last_cmd_time, cmd_interval):
            self._send_cmd(mode=12, gait_id=0, vel=[0.0, 0.0, 0.0])
            last_cmd_time = current_time
            self.app_state.state_id = 1
            print('0 进入状态1')
        return last_cmd_time
    
    # def _handle_state_1(self, current_time, last_cmd_time, cmd_interval):
    #     if self._tick_ready(current_time, last_cmd_time, cmd_interval):
    #         # self._send_cmd(mode=11, gait_id=27, vel=[0.0, 0.0, 0.0])
    #         # self._send_cmd(mode=11, gait_id=27, vel=[0.0, 0.1, 0.0])
    #         # self._send_cmd(mode=11, gait_id=27, vel=[0.01, 0.1, 0.0])
    #         # self._send_cmd(mode=12, gait_id=0, vel=[0.0, 0.0, 0.0])
            
    #         # self._send_cmd(mode=11, gait_id=10, vel=[0.01, 0.1,0])
    #         # self._send_cmd(mode=11, gait_id=10, vel=[0.1, 0.1, 0])
    #         # self._send_cmd(mode=11, gait_id=10, vel=[0.1, 0.15, 0])

    #         # time.sleep(2.5)

    #         # self._send_cmd(mode=11, gait_id=10, vel=[0.1, 0.1, 2.5])
    #         # self._send_cmd(mode=11, gait_id=10, vel=[0.01, 0.1, 0])
    #         self._send_cmd(mode=11, gait_id=10, vel=[0, 0, 2.5])
    #         time.sleep(1.25)
    #         # self._send_cmd(mode=12, gait_id=0, vel=[0.0, 0.0, 0.0])
    #         # time.sleep(3)

    #         # self._send_cmd(mode=11, gait_id=27, vel=[0.01, 0, 2])
    ...
    #         self.app_state.state_id = 2
    #     return last_cmd_time



    def _handle_state_1(self, current_time, last_cmd_time, cmd_interval):
        last_cmd_time, finished = self._turn_by_yaw(
            current_time=current_time,
            last_cmd_time=last_cmd_time,
            cmd_interval=cmd_interval,
            turn_key="state5_forward_turn_90",
    ...
    def _handle_state_2(self, current_time, last_cmd_time, cmd_interval):
        if self._tick_ready(current_time, last_cmd_time, cmd_interval):
            ...
            # self._send_cmd(mode=11, gait_id=10, vel=[0.01, 0, 0])
    ...
    def _handle_state_lane_center(self, current_time, last_cmd_time, cmd_interval):
        ...
        if self._tick_ready(current_time, last_cmd_time, cmd_interval):
            ...
            # 1) 死区：误差很小时直接视为 0，避免来回抖
            if abs(yaw_error) < 0.03:
                yaw_error = 0.0

            # 2) 分段比例：小误差时减小 Kp，避免临近中心时冲过头
